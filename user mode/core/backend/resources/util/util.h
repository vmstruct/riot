...
namespace util
{
			...
			if ( !_stricmp ( current_module_name.c_str ( ) , module_name ) )
			{
				...
				full_path.replace (
					...
					[ ] ( ) {
						...
						size_t len;
						if ( _dupenv_s ( &system_root , &len , "SYSTEMROOT" ) == 0 && system_root != nullptr ) {
							std::string replacement ( system_root );
							free ( system_root );
	...
	inline std::wstring ansi_to_wstring( const std::string& input , DWORD locale = CP_ACP )
	{
	...
	inline std::string wstring_to_ansi( const std::wstring& input , DWORD locale = CP_ACP )
	{
